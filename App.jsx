            ...
            return (
              <Card>
                ...
                <div style={{ display: "flex", gap: "1rem", flexWrap: "wrap" }}>
                  ...
                  {/* Lease */}
                  {(() => {
                    // Mobiliteitsvergoeding wordt bruto ingezet voor de lease
                    // (werkgever betaalt bruto, dat gaat direct naar leasemaatschappij)
                    const leaseNaMobBruto   = Math.max(leasePrive - mobBrutoMaand, 0);
                    const resteigenBijLease = leaseNaMobBruto + bijtellingBelasting;
                    const totaalLeaseNetto  = resteigenBijLease * looptijdMnd;
                    return (
                      <div style={{ flex: 1, minWidth: 240 }}>
                        ...
                        <div style={{ background: "#f7f6f2", borderRadius: 8, padding: "12px 14px" }}>
                          ...
                          <RegelItem label="Mobiliteitsvergoeding (bruto)"
                            waarde={`− ${fmt(mobBrutoMaand)}/mnd`}
                            kleur={COLORS.success}
                            sub="werkgever betaalt direct" />
                          <div style={{ borderTop: "0.5px solid #e0ddd8", margin: "8px 0" }} />
                          <RegelItem label="Eigen bijdrage lease"
                            waarde={`${fmt(leaseNaMobBruto)}/mnd`} />
                          <RegelItem label={`Bijtelling ${state.bijtellingPct}% → belasting`}
                            waarde={`+ ${fmt(bijtellingBelasting)}/mnd`}
                            kleur={COLORS.danger}
                            sub={`${fmt(bijtellingMaand)}/mnd × ${state.belastingschijf}%`} />
                          ...
                          <div style={{ display: "flex", justifyContent: "space-between", fontWeight: 600, fontSize: 15 }}>
                            <span>Netto/mnd</span>
                            <span style={{ color: COLORS.lease }}>{fmt(resteigenBijLease)}</span>
                          </div>
                          <div style={{ display: "flex", justifyContent: "space-between", fontSize: 12, color: "#999", marginTop: 4 }}>
                            <span>Totaal {looptijdMnd} mnd</span>
                            <span>{fmt(totaalLeaseNetto)}</span>
                          </div>
                        </div>
                        {mobBrutoMaand === 0 && (
                          <div style={{ marginTop: 8, fontSize: 12, color: "#999", padding: "6px 10px", background: "#f7f6f2", borderRadius: 6 }}>
                            ⓘ Vul een mobiliteitsvergoeding in op tab "Mijn auto" om het leasevoordeel te zien.
                          </div>
                        )}
                      </div>
                    );
                ...
                {/* Visuele vergelijking + conclusie */}
                {(() => {
                  const leaseNaMobBruto  = Math.max(leasePrive - mobBrutoMaand, 0);
                  const leaseNettoCor    = leaseNaMobBruto + bijtellingBelasting;
                  const totaalLeaseNetto = leaseNettoCor * looptijdMnd;
                  const verschilMaandCor = eigenNettoMaandPeriode - leaseNettoCor;
                  const eigenGoedkoperCor = verschilMaandCor < 0;
                  const totaalVerschilCor = Math.abs(eigenNettoMaandPeriode * looptijdMnd - totaalLeaseNetto);
                    ...
                    <>
                      <div style={{ marginTop: "1.25rem" }}>
                        {[
                          ...
                          { label: "Privé lease netto", maand: leaseNettoCor, color: COLORS.lease },
                        ].map(item => {
                          const max = Math.max(Math.max(eigenNettoMaandPeriode, 0), leaseNettoCor, 1);
                          return (
                      ...
                      <div style={{ marginTop: "1rem", padding: "14px 16px", background: eigenGoedkoperCor ? "#f0faf4" : "#fdf3ef", borderRadius: 8 }}>
                        <div style={{ fontWeight: 600, fontSize: 15, marginBottom: 6 }}>
                          {eigenGoedkoperCor
                            ? `✅ Eigen auto is netto ${fmt(Math.abs(verschilMaandCor))}/mnd goedkoper`
                            : `📋 Lease is netto ${fmt(Math.abs(verschilMaandCor))}/mnd goedkoper`}
                        </div>
                        <div style={{ fontSize: 13, color: "#666", display: "flex", gap: 20, flexWrap: "wrap" }}>
                          <span>Over {looptijdMnd} maanden: verschil <b>{fmt(totaalVerschilCor)}</b></span>
